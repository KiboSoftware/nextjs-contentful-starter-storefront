const formatContentfulProduct = (cmsProducts: any) => {
  if (cmsProducts?.product_recommendations) {
    return {
      ...cmsProducts,
      product_recommendations: cmsProducts?.product_recommendations?.map(
    ...
    return {
      ...cmsProducts,
      customers_also_bought: cmsProducts?.customers_also_bought?.map(
        (productCode: string) => productCode
      ),
    }
  }
...
const formatHomePageProductsData = (homePageProductsData: any) => {
  const homePageProductsReference = homePageProductsData?.reference.map((reference: any) => {
    return {
      ...reference,
      home_page_products: reference.home_page_products?.map((productCode: string) => productCode),
    }
  })
